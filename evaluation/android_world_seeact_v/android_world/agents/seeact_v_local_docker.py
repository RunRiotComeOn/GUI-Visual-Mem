...
######################## add current date and time requirement ########################
question_description = '''Please generate the next move according to the UI screenshot, instruction and previous actions.\n\nInstruction: {}\n\nInteraction History: {}\n'''


system_prompt = "You are a GUI agent. You are given a task and a screenshot of the screen. You need to perform a series of actions to complete the task. Available actions include 'Click', 'Write', 'Scroll', 'LongPress', 'Back', 'Home', 'Wait', 'Terminate', 'Answer', and 'OpenAPP'."

GUIDANCE = (
    '''Here are some useful guidelines you must follow:
General:
- Today's date is 2023-10-15. Pay attention to time-related requirements in the instruction. If an action fails or is ineffective, try reflection and alternative strategies.
...
class SeeAct_V(EnvironmentInteractingAgent):
    ...
    def step(self, goal: str) -> base_agent.AgentInteractionResult:
        ...
        query = question_description.format(goal, history)
        messages = [
                    {"role":"system",
                        "content":[{"type":"text",
                                    "text": system_prompt}]},
...
Action: {{"action_type": "status", "goal_status": "infeasible"}}"""
        ...
        action_match = re.search(r"action_description\":\s*\"(.*?)\"action_type\":", action_output, re.DOTALL)
        action = action_match.group(1).strip().strip(',').strip('"') if action_match else ""

        action_type_match = re.search(r"action_type\":\s*\"(.*?)\"action_target\":", action_output, re.DOTALL)
        action_type = action_type_match.group(1).strip().strip(',').strip('"') if action_type_match else ""

        element_match = re.search(r"action_target\":\s*\"(.*?)\"value\":", action_output, re.DOTALL)
        element_description = element_match.group(1).strip().strip(',').strip('"') if element_match else ""

        value_match = re.search(r"value\":\s*\"(.*?)\"", action_output, re.DOTALL)
        ...
        if action_type == 'open_app':
            ...
            if 'scroll right' in action.lower():
                action_dict['direction'] = 'right'
            elif 'scroll left' in action.lower():
                action_dict['direction'] = 'left'
            elif 'scroll up' in action.lower():
                action_dict['direction'] = 'up'
            elif 'scroll down' in action.lower():
                action_dict['direction'] = 'down' 
            elif 'swipe left' in action.lower():
                action_dict['direction'] = 'right'
            elif 'swipe right' in action.lower():
                action_dict['direction'] = 'left'
            elif 'swipe down' in action.lower():
                action_dict['direction'] = 'up'
            else:
                action_dict['direction'] = 'down'
